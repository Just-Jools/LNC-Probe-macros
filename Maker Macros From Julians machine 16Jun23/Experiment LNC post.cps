...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "boring":
      ...
      writeBlock(
        mFormat.format(510),
        "X" + xyzFormat.format((x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)) * (-1)),
        getProbingArguments(cycle, probeWorkOffsetCode)
        );
      break;
    ...
    case "probing-y":
      ...
      writeBlock(
        mFormat.format(510),
        "Y" + xyzFormat.format((x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)) * (-1)),
        getProbingArguments(cycle, probeWorkOffsetCode)
        );
      break;
    case "probing-z":
      ...
      writeBlock(
        mFormat.format(510),
        "Z" + xyzFormat.format((z - cycle.depth) * (-1)),
        getProbingArguments(cycle, probeWorkOffsetCode)
        );
      break;
    case "probing-x-wall":
    ...
    case "probing-xy-inner-corner":
      ...
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        mFormat.format(10815), xOutput.format(cornerX), yOutput.format(cornerY),
    ...
    case "probing-xy-outer-corner":
      ...
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        mFormat.format(10816), xOutput.format(cornerX), yOutput.format(cornerY),
        conditional(cornerI != 0, "I" + xyzFormat.format(cornerI)),
        conditional(cornerJ != 0, "J" + xyzFormat.format(cornerJ)),
        "Q" + xyzFormat.format(cycle.probeOvertravel),
        getProbingArguments(cycle, probeWorkOffsetCode)
